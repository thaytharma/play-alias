...
function App() {
  ...
  const handleWordChange = () => {
    setWord(isEnglish ? randomEnglishWord() : randomNorwegianWord.getTilfeldigOrd().split(' ')[0]
    );
  }
  ...
  return (
    <div className="App">
      <header className="App-header">
        <h1 className='word' onClick={() => {handleWordChange()}}>
          {word}
        </h1>
        <div className='language-button'  onClick={() => {handleChangeLanguage()}}>{getIcon()}</div>
      </header>
    </div>
  );
  ...
  function getIcon() {
    ...
    return <svg xmlns="http://www.w3.org/2000/svg" id="flag-icons-no" viewBox="0 0 640 480">
